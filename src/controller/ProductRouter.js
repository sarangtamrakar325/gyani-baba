...
let mongo = require('mongodb')    // here this mondo is used for only the calling the mongo object id
...
function router(menu)
{
    ...
    //     data = {
    //        title:"Product Page",
    //        products,     //  === products : products  its called the data destructuring in js 
    //        menu
    //     };
    ...
    productRouter.route('/').get(async (req, res)=>{
      ...
      productRouter.route('/category/:id').get(async (req, res)=>{
        let cid = req.params.id; // params data get from req
        let query = {'category_id': Number(cid)};
        const products = await getData('products', query);
        data = {
           title:"Product Page",
           products,     //  === products : products  its called the data destructuring in js 
      ...
      productRouter.route('/details/:id').get(async (req, res)=>{
        let pid = req.params.id; // params data get from req
        let query = {'id': Number(pid)};
        const products = await getData('products', query);
        data = {
    ...
    productRouter.route('/detail/:id')
        .get(async function(req,res){
            ...
            let {id} = req.params;  // this is called the destructuring of the route
            let query = {_id:mongo.ObjectId(id)}  // here i am passing the mongo object id to the query
            let products = await getData('products',query)
            res.render('products',{title:'Products Detail Page',products,menu})
    })
